...
class Request:
    ...
    def getParam(self, key: str, required=False) -> str:
...
class EasyServerHandler(BaseHTTPRequestHandler):
    ...
    error_message_format = _ERROR_MESSAGE_FORMAT
    ...
    def find_and_call_api_listener(self, path: str, param: dict, listeners_dic):
        ...
        if listener is not None:
            ...
            if current_none_session and request.getSession() is not None:
                # Fixme: may generate same code at a very low rate :-)
                new_session_code = str(uuid.uuid1()).replace("-", "")

                # timeout
                def timeout():
                    with self.session_lock:
                        self.server.sessions.remove(new_session_code)

                s = sched.scheduler()
                s.enter(12 * 3600, 1, timeout)
                self.server.sessions[new_session_code] = request.getSession()
                self.send_header("Set-Cookie", self.SESSION_COOKIE_NAME + "=" + new_session_code + "; path=/;")
            self.end_headers()
    ...
    def deal_static_file_request(self, path):
        ...
        content_type = default_type if len(postfix) == len(path) else _extensions_map.get(postfix, default_type)
        try:
            f = open(path, 'rb')
        except OSError:
